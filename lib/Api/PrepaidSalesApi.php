...
class PrepaidSalesApi
{
     ...
     * @throws ApiException on non-2xx response
     * @return \Secuconnect\Client\Model\PrepaidSalesList
     */
     ...
     * @param string $preset Query presets 
     * @throws ApiException on non-2xx response
     * @return array of \Secuconnect\Client\Model\PrepaidSalesList, HTTP status code, HTTP response headers (array of strings)
     */
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    $httpBody,
                    $headerParams,
                    '\Secuconnect\Client\Model\PrepaidSalesList',
                    '/Prepaid/Sales'
                );

                return [$this->apiClient->getSerializer()->deserialize($response, '\Secuconnect\Client\Model\PrepaidSalesList', $httpHeader), $statusCode, $httpHeader];
            } catch (ApiException $e) {
                switch ($e->getCode()) {
                    case 200:
                        $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Secuconnect\Client\Model\PrepaidSalesList', $e->getResponseHeaders());
                        $e->setResponseObject($data);
                        break;
                    case 401:
                        if ($retries < 1) {
    ...
    public function getOneWithHttpInfo($prepaid_sale_id)
    {
        ...
        if ($prepaid_sale_id === null || (is_array($prepaid_sale_id) && count($prepaid_sale_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $prepaid_sale_id when calling getOne'
            );
        }
        // parse inputs
        $resourcePath = "/Prepaid/Sales/{prepaidSaleId}";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
