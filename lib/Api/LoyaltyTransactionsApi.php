...
class LoyaltyTransactionsApi
{
     ...
     * @return \Secuconnect\Client\Model\LoyaltyTransactionsList
     */
     ...
     * @param string $expand Expand fields (all or a specific one) 
     * @throws ApiException on non-2xx response
     * @return array of \Secuconnect\Client\Model\LoyaltyTransactionsList, HTTP status code, HTTP response headers (array of strings)
     */
    ...
    {
        // parse inputs
        $resourcePath = "/Loyalty/Transactions";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    $httpBody,
                    $headerParams,
                    '\Secuconnect\Client\Model\LoyaltyTransactionsList',
                    '/Loyalty/Transactions'
                );

                return [$this->apiClient->getSerializer()->deserialize($response, '\Secuconnect\Client\Model\LoyaltyTransactionsList', $httpHeader), $statusCode, $httpHeader];
            } catch (ApiException $e) {
                switch ($e->getCode()) {
                    case 200:
                        $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Secuconnect\Client\Model\LoyaltyTransactionsList', $e->getResponseHeaders());
     ...
     */
    public function getOne($loyalty_transaction_id, $expand = null)
    {
        list($response) = $this->getOneWithHttpInfo($loyalty_transaction_id, $expand);
        return $response;
    }
     ...
     */
    public function getOneWithHttpInfo($loyalty_transaction_id, $expand = null)
    {
        // verify the required parameter 'loyalty_transaction_id' is set
        if ($loyalty_transaction_id === null || (is_array($loyalty_transaction_id) && count($loyalty_transaction_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $loyalty_transaction_id when calling getOne'
            );
        }
        // parse inputs
        $resourcePath = "/Loyalty/Transactions/{loyaltyTransactionId}";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        // path params
        if ($loyalty_transaction_id !== null) {
            $resourcePath = str_replace(
                "{" . "loyaltyTransactionId" . "}",
                $this->apiClient->getSerializer()->toPathValue($loyalty_transaction_id),
                $resourcePath
            );
        }
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    $httpBody,
                    $headerParams,
                    '\Secuconnect\Client\Model\LoyaltyTransactionsProductModel',
                    '/Loyalty/Transactions/{loyaltyTransactionId}'
                );

                return [$this->apiClient->getSerializer()->deserialize($response, '\Secuconnect\Client\Model\LoyaltyTransactionsProductModel', $httpHeader), $statusCode, $httpHeader];
            } catch (ApiException $e) {
                switch ($e->getCode()) {
                    case 200:
                        $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Secuconnect\Client\Model\LoyaltyTransactionsProductModel', $e->getResponseHeaders());
                        $e->setResponseObject($data);
                        break;
                    case 401:
                        if ($retries < 1) {
