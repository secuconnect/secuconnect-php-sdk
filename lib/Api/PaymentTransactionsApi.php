...
class PaymentTransactionsApi
{
     ...
     */
    public function assignPayment($payment_transaction_id, $accounting_id)
    {
        list($response) = $this->assignPaymentWithHttpInfo($payment_transaction_id, $accounting_id);
        return $response;
    }
     ...
     */
    public function assignPaymentWithHttpInfo($payment_transaction_id, $accounting_id)
    {
        ...
        if ($payment_transaction_id === null || (is_array($payment_transaction_id) && count($payment_transaction_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $payment_transaction_id when calling assignPayment'
            );
        }
        // verify the required parameter 'accounting_id' is set
        if ($accounting_id === null || (is_array($accounting_id) && count($accounting_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $accounting_id when calling assignPayment'
            );
        }
        // parse inputs
        $resourcePath = "/Payment/Transactions/{paymentTransactionId}/assignPayment/{accountingId}";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        if ($accounting_id !== null) {
            $resourcePath = str_replace(
                "{" . "accountingId" . "}",
                $this->apiClient->getSerializer()->toPathValue($accounting_id),
                $resourcePath
            );
        }
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    '/Payment/Transactions/{paymentTransactionId}/assignPayment/{accountingId}'
                );
     ...
     */
    public function getRefundBankAccount($body)
    {
        list($response) = $this->getRefundBankAccountWithHttpInfo($body);
        return $response;
    }
     ...
     */
    public function getRefundBankAccountWithHttpInfo($body)
    {
        // parse inputs
        $resourcePath = "/Payment/Transactions/me/getRefundBankAccount";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    $httpBody,
                    $headerParams,
                    '\Secuconnect\Client\Model\PaymentTransactionsGetRefundBankAccount',
                    '/Payment/Transactions/me/getRefundBankAccount'
                );

                return [$this->apiClient->getSerializer()->deserialize($response, '\Secuconnect\Client\Model\PaymentTransactionsGetRefundBankAccount', $httpHeader), $statusCode, $httpHeader];
            } catch (ApiException $e) {
                switch ($e->getCode()) {
                    case 200:
                        $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Secuconnect\Client\Model\PaymentTransactionsGetRefundBankAccount', $e->getResponseHeaders());
                        $e->setResponseObject($data);
                        break;
                    case 401:
                        if ($retries < 1) {
