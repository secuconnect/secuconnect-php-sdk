...
class CardprocessingInvoicesApi
{
     ...
     * @return \Secuconnect\Client\Model\CardprocessingInvoicesList
     */
     ...
     * @param string $expand Expand fields (all or a specific one) 
     * @throws ApiException on non-2xx response
     * @return array of \Secuconnect\Client\Model\CardprocessingInvoicesList, HTTP status code, HTTP response headers (array of strings)
     */
    ...
    {
        // parse inputs
        $resourcePath = "/Cardprocessing/Invoices";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    $httpBody,
                    $headerParams,
                    '\Secuconnect\Client\Model\CardprocessingInvoicesList',
                    '/Cardprocessing/Invoices'
                );

                return [$this->apiClient->getSerializer()->deserialize($response, '\Secuconnect\Client\Model\CardprocessingInvoicesList', $httpHeader), $statusCode, $httpHeader];
            } catch (ApiException $e) {
                switch ($e->getCode()) {
                    case 200:
                        $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Secuconnect\Client\Model\CardprocessingInvoicesList', $e->getResponseHeaders());
     ...
     */
    public function getOne($cardprocessing_invoice_id, $expand = null)
    {
        list($response) = $this->getOneWithHttpInfo($cardprocessing_invoice_id, $expand);
        return $response;
    }
     ...
     */
    public function getOneWithHttpInfo($cardprocessing_invoice_id, $expand = null)
    {
        // verify the required parameter 'cardprocessing_invoice_id' is set
        if ($cardprocessing_invoice_id === null || (is_array($cardprocessing_invoice_id) && count($cardprocessing_invoice_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $cardprocessing_invoice_id when calling getOne'
            );
        }
        // parse inputs
        $resourcePath = "/Cardprocessing/Invoices/{cardprocessingInvoiceId}";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        // path params
        if ($cardprocessing_invoice_id !== null) {
            $resourcePath = str_replace(
                "{" . "cardprocessingInvoiceId" . "}",
                $this->apiClient->getSerializer()->toPathValue($cardprocessing_invoice_id),
                $resourcePath
            );
        }
        ...
        for ($retries = 0; ; $retries++) {
            ...
            try {
                list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                    ...
                    $httpBody,
                    $headerParams,
                    '\Secuconnect\Client\Model\CardprocessingInvoicesProductModel',
                    '/Cardprocessing/Invoices/{cardprocessingInvoiceId}'
                );

                return [$this->apiClient->getSerializer()->deserialize($response, '\Secuconnect\Client\Model\CardprocessingInvoicesProductModel', $httpHeader), $statusCode, $httpHeader];
            } catch (ApiException $e) {
                switch ($e->getCode()) {
                    case 200:
                        $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Secuconnect\Client\Model\CardprocessingInvoicesProductModel', $e->getResponseHeaders());
                        $e->setResponseObject($data);
                        break;
                    case 401:
                        if ($retries < 1) {
